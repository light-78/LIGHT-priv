...
/******** OWNER NUMBER**********/
const ownerNumber = ["33622007411@s.whatsapp.net","556181496039@s.whatsapp.net"] 
/************************************/
...
pp_riu = await getBuffer(pp_user)
		         ...
		         reply(`[❗] Olá ${pushname} aguarde⏳`)                 
					num = `${sender.split("@")[0]}@s.whatsapp.net`
		    ow = "+33622007411@s.whatsapp.net"
		    cuy = "+336220074 11@s.whatsapp.net"
                    menusu = `
❏ *MENU DE FERRAMENTAS*
│by light
...
break
				...
				reply(`[❗] mandando E ENVIANDO SUA MÚSICA...`)                             
                reply(`[❗] ESPERE...`)
...
break
				...
				case 'dorothy':
					...
					if (!isBotGroupAdmins) return reply(ind.badmin())
					if (args[0] === 'a') {
					    reply(`*❗️GRUPO ABIERTO❗*`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, false)
					} else if (args[0] === 'f') {
						reply(`*❗GRUPO ABRE  mañanaAS 6:00❗*️`)
						client.groupSettingChange(from, GroupSettingChange.messageSend, true)
					}
				break      
           ...
           case 'rebaixar':
				...
				case 'bemvindo':
					...
					if (Number(args[0]) === 1) {
						...
						reply('❬ ✅ ❭ ACTIVADO COM SUCESSO O MODO-BOAS VINDAS️ NESTE GRUPO')
					} else if (Number(args[0]) === 0) {
						welkom.splice(from, 1)
						fs.writeFileSync('./database/bot/welkom.json', JSON.stringify(welkom))
						reply('❬ ❌ ❭ DESACTIVADO COM SUCESSO O MODO-BOAS VINDAS️ NESTE GRUPO')
					} else {
						reply(ind.satukos())
					}
					break 
     ...
     client.updatePresence(from, Presence.composing) 
				...
				case 'antilink':
					...
					if (Number(args[0]) === 1) {
						...
						reply('*Atencao a todos os membros ativos deste grupo o anti-link esta ativado si vos enviar algum link, usted sera expulsado del grupo*')
					} else if (Number(args[0]) === 0) {
